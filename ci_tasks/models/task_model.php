...
class Task_model extends CI_Model {
  ...
  public function get_tasks($user_id, $layout, $uri_date = NULL) {
    ...
    $query =
      "SELECT * FROM (
        SELECT categories.category, categories.column, tasks.id as task_id, tasks.task, tasks.important, tasks.link_href, tasks.link_text, tasks.order, categories.id as cat_id, tasks.completed, categories.user_id, categories.display, categories.order as cat_order
        FROM tasks
    ...
    if($layout == 1) {
      ...
      foreach($rows as $row) {
        ...
        $data[$row->cat_id]['cat_order'] = $row->cat_order;
      ...
      foreach($rows as $row) {
        ...
        $data[$row->column][$row->cat_id]['cat_order'] = $row->cat_order;
